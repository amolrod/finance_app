...
export function MacbookScroll({ src, showGradient, title, badge, children }: MacbookScrollProps) {
  ...
  const [translateDistance, setTranslateDistance] = useState(900);

  useEffect(() => {
    const update = () => {
      const mobile = window.innerWidth < 768;
      const height = window.innerHeight || 800;
      const maxDistance = mobile ? 700 : 900;
      const distance = Math.min(maxDistance, Math.round(height * (mobile ? 0.85 : 1)));
      setIsMobile(mobile);
  ...
  const translate = useTransform(scrollYProgress, [0, 0.75, 1], [0, translateDistance, translateDistance]);
  ...
  return (
    <div
      ref={ref}
      className="flex min-h-[200vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100 md:pt-56 md:pb-80"
    >
      <motion.h2
        style={{
...
export function Lid({ scaleX, scaleY, rotate, translate, src, children }: LidProps) {
  return (
...
export function KBtn({ className, children, childrenClassName, backlit = true }: KBtnProps) {
  return (
    <div
      className={cn(
